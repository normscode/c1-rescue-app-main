...
@RoutePage()
class AdminIncidentListScreen extends StatefulWidget {
  const AdminIncidentListScreen({super.key});

  @override
  State<AdminIncidentListScreen> createState() => _AdminIncidentListScreenState();
}

class _AdminIncidentListScreenState extends State<AdminIncidentListScreen> {
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: const TabBar(
            tabs: [
              Tab(text: "All", icon: Icon(Icons.on_device_training_outlined)),
...
class IncidentItem extends StatefulWidget {
  ...
  const IncidentItem(this.data, {super.key});
      ...
      builder: (context, snapshot) {
        ...
        return Padding(
          ...
          child: GestureDetector(
            onTap: () {
              context.router.push(IncidentViewRoute(data: widget.data));
            },
            child: Container(
...
class _EmergencyTabState extends State<EmergencyTab> {
  ...
  void initState() {
    ...
    if (widget.tabName == "all") {
      read = context.read<EmergencyListCubit>().getIncidents();
    } else if (widget.tabName == "unsolved") {
      read = context.read<EmergencyListCubit>().getUnsolved();
    } else if (widget.tabName == "resolved") {
      read = context.read<EmergencyListCubit>().getResolved();
    } else {
